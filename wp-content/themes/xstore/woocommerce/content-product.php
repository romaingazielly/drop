...
if ( in_array('product_page_addtocart', $product_settings) && ! etheme_is_catalog() ) {
	$classes[] = 'et_cart-on';
}
else {
	$classes[] = 'et_cart-off';
...
if ( etheme_get_option('product_title_limit_type', 'chars') == 'chars' ) {
			...
			<?php if ( $view == 'custom' && $custom_template != '' ) {
				...
				<div class="product-image-wrapper hover-effect-<?php echo esc_attr( $hover ); ?>">
					...
					<?php if ( $view == 'booking' && $view_mode != 'list' ): ?>
						...
						<?php
						if (in_array('product_page_addtocart', $product_settings) && $view != 'booking' && ! etheme_is_catalog() ) {
